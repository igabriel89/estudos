		...
		while (events.hasNext()) {
			...
			try {
				if (exception instanceof ViewExpiredException) {
					handled = true;
					redirect("/home.xhtml");
					System.out.println("Clase JsfExcHander condicional 1");
					System.out.println("------------------------------------");
				} else if (negocioException != null) {
					handled = true;
					FacesUtil.addErrorMessage(negocioException.getMessage());
					System.out.println("Clase JsfExcHander condicional 2");
					System.out.println("------------------------------------");
				} else {
					handled = true;
					redirect("/Erro.xhtml");
					log.error("Erro de sistema: " + exception.getMessage(), exception);
					System.out.println("Clase JsfExcHander condicional 3");
					System.out.println("------------------------------------");
				}
			} finally {
				if (handled) {
